...
<div class="container over">
    <div class="section ">
      <div class="row ">
        ...
        <div class="row">
          <div class="input-field col s10 offset-s1" style="position: relative;">
            <!--<span style="position: absolute; top: 65%; left: 30px; transform: translateY(-50%); color: grey;">https://</span>-->
            <input id="weburl" name="weburl" type="url" class="validate" style="padding-left: 70px;" oninput="validateURL()">
            <label for="weburl">URL</label>
            <span id="url-error" style="padding-left: 20px;color: orange; font-size: 12px; display: none;">Please enter a valid URL.</span>
        ...
        <div class="row">
          <div class="input-field col s10 offset-s1" style="position: relative;">
            <!--<span style="position: absolute; top: 50%; left: 10px; transform: translateY(-50%); color: grey;">@</span>-->
            <input id="email" name="email" type="text" class="validate" style="padding-left: 70px;" disabled>
            <label for="email">Email</label>
            <span id="emailError" style="padding-left: 20px;color: orange; font-size: 12px; display: none;">Email domain must match the URL domain.</span>
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  fetch('fetch_categories.php')
    ...
    .then((data) => {
      ...
      // Re-initialize the Materialize select
  ...
  // URL Validation and Email Field Updates
  ...
  function validateURL() {
    try {
      ...
      if (!/^https?:\/\//i.test(url)) {
        url = `https://${url}`;
        urlField.value = url; // Update the field value with the corrected URL
      }
      ...
      if (domain) {
        ...
        emailField.value = ""; // Reset email input
        emailField.placeholder = `Enter your email (e.g., name@${domain})`;
        emailField.setAttribute("data-domain", domain); // Store the domain for email validation
  ...
  // Trigger URL validation on input
  urlField.addEventListener("input", validateURL);

  // Handle email input and append domain dynamically
  emailField.addEventListener("input", function (e) {
    const domain = this.getAttribute("data-domain");
    if (domain) {
      // Split the input value into the local part and domain
      const parts = this.value.split("@");
      const localPart = parts[0].replace(/[^a-zA-Z0-9._%+-]/g, ""); // Sanitize local part
      this.value = localPart + "@" + domain;
    }
  });

  // Prevent further user typing after the domain
  emailField.addEventListener("keydown", function (e) {
    const domain = this.getAttribute("data-domain");
    if (domain && this.value.includes("@" + domain)) {
      // Block input if cursor is after the domain part
      const cursorPosition = this.selectionStart;
      const domainIndex = this.value.indexOf("@" + domain);
      if (cursorPosition > domainIndex) {
        e.preventDefault();
      }
    }
  });

  // Validate email domain matches URL domain
  emailField.addEventListener("blur", function () {
    const emailDomain = emailField.value.split("@")[1];
    const urlDomain = emailField.getAttribute("data-domain");
