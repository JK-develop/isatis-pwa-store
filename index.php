...
<div class="content-area">
  ...
  <div class="carousel carousel-fullscreen carousel-slider home_carousel ">
    ...
    <!--    <a class="carousel-item" target="_blank" href="https://isatis.vip/JK-swf">-->
    <!--  <div class="bg" style="background-image: url('assets/images/slide_jk_swf.jpg')"></div>-->
    <!--  <div class="item-content center-align white-text">-->
    <!--     <div class="spacer"></div>-->
    <!--    <h3>SWF viewer app -> online Flash player</h3>-->
    <!--          </div>-->
    <!--</a>-->
  </div>
  </div>
...
$statement = $connect->prepare("SELECT * FROM applications WHERE status = 'active' ORDER BY RAND(), id DESC;");
  ...
  $total_rows = $statement->rowCount();
        ...
        $fullStars = floor($vote); // Number of full stars
        $halfStar = ($vote - $fullStars >= 0.5) ? 1 : 0; // Check if there's a half star
        $emptyStars = 5 - ($fullStars + $halfStar); // Remaining stars will be empty
        // $voteResult = '<div class="prod-rating tooltipped" data-position="top" data-tooltip="'.$vote.'">';
        $voteResult = '<div class="prod-rating">';
        // Render full stars
        for ($i = 0; $i < $fullStars; $i++) {
            $voteResult .= '<i class="mdi mdi-star colored"></i>';
        }
        // Render half star if applicable
        if ($halfStar) {
            $voteResult .= '<i class="mdi mdi-star-half colored"></i>';
        }
        // Render empty stars
        for ($i = 0; $i < $emptyStars; $i++) {
            $voteResult .= '<i class="mdi mdi-star-outline colored"></i>';
        }
        $voteResult .= '</div>';
        ...
        $formattedDateAlt = date('F j, Y', $timestamp); 

        // GET CATEGORIES
        // Prepare the SQL query
        $statementListCategories = $connect->prepare(
            "SELECT c.title 
        ...
        $resultCat = $resultCat . '<div class="chip">' . htmlspecialchars($category) . '</div>';
        }

        // GET VERSIONS
        // Prepare the SQL query
        $statementLastversion = $connect->prepare(
            "SELECT v.version, v.url 
             FROM versions v WHERE v.app_id = :app_id"
        );
        // Bind the parameter
        $statementLastversion->bindParam(':app_id', $app_id, PDO::PARAM_INT);
        // Execute the query
        $statementLastversion->execute();
        // Fetch the result as an associative array
        $version = $statementLastversion->fetchAll(PDO::FETCH_ASSOC);
...
<div class="col s12">
    <div class="prod-img-wrap">
        <a class="img-wrap" href="' . htmlspecialchars($app_banner) . '" data-fancybox="images"
           data-caption="' . htmlspecialchars($app_title) . '"
           style="background-image: url(' . htmlspecialchars($app_banner) . ');">&nbsp;</a>
    </div>
    <div class="prod-info boxed">
        <a href="app_view.php?id=' . $app_id . '">
            <h5 class="title truncate">' . htmlspecialchars($app_title) . '</h5>
        </a>
        <span class="small brand">&copy; ' . htmlspecialchars($app_developer) . '</span>
        <p class="bot-0 text">' . htmlspecialchars($app_description) . '</p>
        <div class="spacer-line"></div><div class="spacer-line"></div><hr/>Categories:<br/>
        '.$resultCat.'<hr/>
...
'.$voteResult.'
    ...
    <a class="img-wrap round" href="assets/images/portfolio-6.jpg" data-fancybox="images" 
       data-caption="No record found!">
        <img alt="image" class="" style="width: 100%;" src="assets/images/portfolio-6.jpg">
    </a>
</div>';
        }
    } catch(PDOException $error) {  
...
<!-- OTHER SCRIPTS INCLUDED ON THIS PAGE - START -->
<script src="modules/fancybox/jquery.fancybox.min.js" ></script><script src="assets/js/common.js"></script><!-- OTHER SCRIPTS INCLUDED ON THIS PAGE - END -->
