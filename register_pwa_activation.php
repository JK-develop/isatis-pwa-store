...
require 'db_connection.php'; // Replace with your database connection script
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['register'])) {
    ...
    $categories = $_POST['category']; // Array of category IDs
    $description = htmlspecialchars($_POST['description']);
    $version = htmlspecialchars($_POST['version']);
    $confirmation_code = rand(100000, 999999); // Generate a random 6-digit code
    ...
    try {
        ...
        if ($existingUser) {
            ...
            $message = "User already exists. USER ID: " . $user_id;
        } else {
            // INSERT USER
            ...
            $stmt->execute([
                ...
                ':rememberToken' => $confirmation_code,
            ]);
    
            $user_id = $connect->lastInsertId();
             $message = "OK - USER ID : " . $user_id;
        }
        
        // Check if an APP with the provided email exists
        $stmt = $connect->prepare("SELECT id FROM applications WHERE developer = :email");
        $stmt->execute([':email' => $email]);
        $existingAPP = $stmt->fetch(PDO::FETCH_ASSOC);
        
        if ($existingAPP) {
            // APP already exists, get the user ID and add new version
            $pwa_id = $existingAPP['id'];
            
                    // Remove existing categories for the application
                    $delete_stmt = $connect->prepare("
                        DELETE FROM application_category 
            ...
            // Insert categories into a junction table
            foreach ($categories as $category_id) {
                $category_stmt = $connect->prepare("
                    INSERT INTO application_category (application_id, category_id)
                    VALUES (:application_id, :category_id)
                ");
            ...
            // INSERT APP VERSION
            $stmt = $connect->prepare("
                INSERT INTO versions (app_id, url, created_at, updated_at)
                VALUES (:appId, :url, :createdAt, :updatedAt)
            ");
            $stmt->execute([
                ':appId' => $pwa_id,
                ':url' => $url,
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['verify'])) {
    ...
    try {
        ...
        // check confirmation code of user
        $stmt = $connect->prepare("
            SELECT * FROM users WHERE email = :email AND remember_token = :confirmation_code
        ");
        $stmt->execute([':email' => $email, ':confirmation_code' => $entered_code]);
        $usr = $stmt->fetch(PDO::FETCH_ASSOC);
        // get relevant app of user
        $stmt = $connect->prepare("
            SELECT * FROM applications WHERE fk_user = :userId
        ");
        $stmt->execute([':userId' => $usr['id']]);
        $pwa = $stmt->fetch(PDO::FETCH_ASSOC);
        if ($pwa) {
            // Update PWA status to active
            $update_stmt = $connect->prepare("UPDATE applications SET status = 'active' WHERE id = :id");
            $update_stmt->execute([':id' => $pwa['id']]);

            // Send publishing license email
            $license_subject = "Your PWA Publishing License";
            $license_message = "Congratulations! Your PWA has been successfully registered.\n\nHere is your publishing license.";
            $license_headers = "From: noreply@isatis.vip";

            mail($email, $license_subject, $license_message, $license_headers);

            $message = "Your PWA has been successfully registered! Check your email for the publishing license.";
...
<div class="container over">
    <div class="section ">
      ...
      <form id="pwaVerifyForm" method="POST" action="#">
...
<div class="footer-menu circular">
  <ul>
      <li >
      <a href="#" >      <i class="mdi mdi-new-box"></i>
      <span>PWA</span>
      </a>    </li>
        <li >
      <a href="#" >      <i></i>
      <span></span>
        ...
        <li>
      <a href="index.php" >      <i class="mdi mdi-apps"></i>
      <span>Home</span>
      </a>    </li>
        <li >
      <a href="#" >      <i></i>
